...
def get_scenario_name(args, env_config_file, policy_config, env, robot):
    ...
    if args.policy == 'campc':
        ...
        if not os.path.isdir(save_dir):
            os.makedirs(save_dir)
        scenario_name = 'NewMPC_{:}_{:}'.format(env_id, args.num_humans)
    elif 'NewMPCChanging' in args.policy:
        save_dir = os.path.join(os.getcwd(), 'results_NewMPCChanging', env_id)
        if not os.path.isdir(save_dir):
            os.makedirs(save_dir)
        scenario_name = 'NewMPCChanging_{:}_{:}'.format(env_id, args.num_humans)
...
def main():
    ...
    # Set the default policy to ORCA Plus
    parser.add_argument('--policy', type=str, default='campc')
    ...
    while not done:
        ...
        #logging.info('[NisalaTestCode] Config {:} = {:}'.format('ob', ob))
        #logging.info('[NisalaTestCode] Config {:} = {:}'.format('Stat_ob', static_obs))
